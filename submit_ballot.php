<?php
	...
	if(isset($_POST['vote'])){
		if(count($_POST) == 1){
			$_SESSION['error'][] = 'Por favor vote pelo menos um candidato';
		}
		else{
			$_SESSION['post'] = $_POST;
				...
				if(isset($_POST[$position])){
					if($row['max_vote'] > 1){
						if(count($_POST[$position]) > $row['max_vote']){
							$error = true;
							$_SESSION['error'][] = 'Você só pode escolher '.$row['max_vote'].' candidatos a '.$row['description'];
						}
						else{
		...
		$_SESSION['error'][] = 'Selecione candidatos para votar primeiro';
	}
